 ...
 *   list_ensure() 가 용량을 2배로 늘리고 realloc 한다. 이 리스트로 큰 수열을 //메모리 재할당?
 *   만들어 합을 구한다.
 *
 * [기대 동작]
...
typedef struct
{
    ...
    size_t len;
    size_t cap;
    size_t new_cap; // l-cap과 갱신과 순서를 맞춰야함 변수 추가
} IntList;
...
static void list_ensure(IntList *l, size_t need)
{
    ...
    int *p = realloc(l->data, l->cap * sizeof(size_t)); // int->size_t 수정
    if (!p)
    {
        perror("realloc");
        free(l->data);
