 ...
 *
 * [시나리오]
 *   직원 레코드(Rec)를 힙에 만들고, "ID 순 인덱스"(by_id)와 "이름 순 인덱스"(by_name)
 ...
 *   (gdb) frame N ; print by_name[i] → 이 주소가 앞서 by_id 로 이미 free 됐는지 확인
 *   (gdb) print dir->by_id[0]        → by_id[0] 와 by_name 의 원소가 같은 객체를 가리킴
 *
 * [printf(로그)로 잡기]
